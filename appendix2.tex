...
\end{lemma}
\begin{proof}
	不妨设 $O \neq \emptyset$. 任取 $\mathcal{C}$ 的投射生成元 $s$. 以余完备的条件取直和
...
\begin{proof}
	...
	\[ \mathcal{A} \to \ProC(\mathcal{A}) \xrightarrow{G} \cated{Mod}R , \quad G := \Hom_{\ProC(\mathcal{A})}(S, \cdot), \; R := \End_{\ProC(\mathcal{A})}(S). \]
	记其合成为 $F$. 因为每段皆正合, $F$ 是正合函子. 应用引理 \ref{prop:FM-prep-2} 可知 $F$ 全忠实. 明所欲证.
\end{proof}
