...
\end{lemma}
\begin{proof}
	定义 \ref{def:DK-Gamma} 蕴涵 $A_n \subset \mathrm{N}\Gamma(A)_n$, 对应到 $\identity: [n] \to [n]$ 在 $\Gamma(A)_n$ 中确定的直和项, 而且 $\mathrm{N}\Gamma(A)_n \to \mathrm{N}\Gamma(A)_{n-1}$ 限制为 $\partial_n: A_n \to A_{n-1}$. 记 $\Gamma(A)_n$ 中其它项之和与 $\mathrm{N}\Gamma(A)_n$ 的交为 $\mathrm{N}'\Gamma(A)_n$, 问题化为证 $\mathrm{N}'\Gamma(A)_n = 0$. 为了方便陈述, 以下设 $\mathcal{A} = \cate{Ab}$, 不影响实质.
